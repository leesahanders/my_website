---
title: "Setting the package repository on Posit Workbench and Connect for R and Python"
description: ""
author: "Lisa"
date: "2025-09-29"
format:
...
Setting the repository for R and Python package installations is a critical part of having a secure system. Malicious package attacks are a [big issue](https://thehackernews.com/2025/04/malicious-python-packages-on-pypi.html), and domain squatting attacks are a new-ish vector for these to get in the door. 

Having a controlled repository with [vulnerable package blocking](https://docs.posit.co/rspm/admin/security/package.html#vulnerability-blocking) is critical. But it's only useful if users are actually using the repository. When using something like Package Manager you'll want users to install packages from there, rather than the broader internet. 
...
## Longer explanation and considerations
...
Parallelize your package installs (particularly useful for monstrous bioconductor packages) using `pak` with `renv`. `pak` will natively use parallelization for the install, making it significantly faster. 
...
pip config set global.trusted-host pkg.demo.posit.team
```

#### Specific project for the current user 
