...
This was a focus topic during Posit::Conf 2025!

For positron assistant and databot we will need credentials to connect to the LLM. In my case, that's AWS Bedrock and it being added to user sessions as part of the AWS infrastructure. We can check from terminal with: `env | grep AWS`


![Positron Assistant](img/positron-assistant.png){#fig-dark-theme}



To use a LLM that isn't supported yet you'll need to go in and modify a couple parameters as described [here](https://connect.posit.it/positron-wiki/positron-assistant.html#aws-bedrock). You'll want to hit the keys command-shift-p to open the settings menu and then click on `open user preference (json)` and update it to have the appropriate settings, which might look like this: 

```{.bash filename="settings.json"}
{
    "extensions.autoCheckUpdates": "false",
    "extensions.autoUpdate": "false",
    "positron.environment.enabled": "false",
    "python.environmentProviders.enable": "map[Conda:false]",
    "python.interpreters.exclude": "[/usr/bin /bin]",
    ...
    "positron.assistant.copilot.enable": true,
...
- Download from the extensions store
- ctrl shift p user settings (not json) and databot authorize 
- the ctrl shift p and open databot 
...
# If you want to test installs and see the repository location
uv pip install seaborn --reinstall -v
```
...
To lock dependencies declared in a pyproject.toml into the requirements.txt file:
...
```bash
# Make sure you are in a directory with your code ("open folder")

# Create the uv project, this example declares the python version to use explicitly. Make sure this matches a version you have access to.
uv init --app --python 3.12.4

# create the venv
uv venv

# Make sure you source it, it will give you the command to run

# Install packages
uv pip install pandas
uv pip install -r requirements.txt

# And then you may need to select your venv as the interpreter and reload the session with ctrl-shift-p reload window

# ctrl shift p to open the command pallette and select the python interpreter that matches the venv we created, or top right where it says python or R select new console session and the venv we created. make sure that bottom left also matches the venv
```
...
The requirements.txt can be locked to a particular repository with: 
