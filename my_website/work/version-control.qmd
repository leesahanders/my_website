...
Set a long timeout on the store so it will work on linux: 
```bash
...
For gitlab generate a PAT token in gitlab with: 

- Log in to your GitLab
- Navigate to your avatar at the top-right corner and click Settings
- In the left sidebar, choose Access Tokens
- Provide a name for your token and an optional expiration date
- Check the api and write_repository scopes
- Click Create personal access token
- Copy the generated token and store it securely

Then go through the normal process for cloning down a project through RStudio. When prompted for a password use the PAT token. 
