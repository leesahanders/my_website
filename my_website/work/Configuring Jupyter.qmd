...
Umask in STIG systems is typically set to 077 (on rhel9) which will make the libraries installed under the python version unable to be used by normal users. Set umask to 022 and then reinstall python and jupyter to fix this. Alternatively could try to fix the file permissions with the -exec flag and chmod, for example with `chmod -R o+rx /opt/python/3.10.8` which would set the permissions to 755. 
